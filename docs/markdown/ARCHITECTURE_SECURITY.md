...
| **Outbound AI gateway** (P5-T02, design P5-T01/#58) | The platform's **second** egress, and the first carrying *tenant business content* rather than a public figure: prompt leakage of customer financial data; provider-side training on tenant data; cross-tenant context bleed; runaway spend against a metered API; a held-open streaming call starving the cron worker | **The "no tenant DB makes an outbound call" rule above still holds, and is not weakened.** Context is built and PII-sanitised **inside the tenant database** — which physically cannot read another tenant's DB — then handed over Odoo's public interfaces (RPC / job queue) to the **AI gateway satellite** (`ARCHITECTURE_DATA_PLATFORM.md` §10.2–10.4: its own container, holding LLM keys but **no DB credentials** — it receives context, never fetches it). That satellite alone performs the outbound call, to a single allowlisted provider host, reusing `config_sync.py`'s bounded-read/deadline/no-redirect patterns (#278/#283/#308/#309). **No database — tenant or admin — is in the egress path.** The gateway is the sole choke point: per-tenant token budget and rate limit, hard spend ceiling, circuit breaker. It logs **metadata only — never prompt or completion bodies** — consistent with the `ncollection_auth` retention stance (#219/#261). Contractual no-training is a hard provider requirement. **Prerequisite: #311** (network-level egress backstop) — a second egress enforced only by code review is not acceptable. Streaming must not run on the cron worker (#310, `max_cron_threads = 1`). Data residency is **not** promised by this row: in-region processing is P10-T05 |
| **Container egress** (all outbound, #311) | Docker wires its own FORWARD rules through the `DOCKER-USER` iptables chain and **bypasses UFW's `default allow outgoing`** — so the code-level controls above (rate fetch #308, AI gateway #58) are the *only* thing stopping a compromised container from reaching the whole internet. A code-review-only egress boundary is not acceptable for the surfaces above. | **Network-level default-deny egress backstop** (`scripts/deploy/harden.sh` step [7/7]): containers run on two fixed-subnet planes (`docker-compose.yml`) — an internal-only db-plane (`nc_dbplane`, no gateway, so `db`/`pgbouncer` cannot reach off-host at all) and an egress plane (`default`, 172.31.240.0/24) whose outbound is filtered in `DOCKER-USER` to an explicit allowlist (`config/hardening/egress_allowlist.txt` — currently ECB, Stripe, ACME; the AI-gateway provider host is a **commented, inactive slot** activated when P5-T02 ships). Everything else DROPs; IPv6 default-denied. Zero-gap A/B chain swap so the policy is never absent during a refresh. This is a *backstop behind* the code controls, not a replacement — it does not track CDN/anycast IP drift (re-run `harden.sh` to refresh; see RUNBOOK_SECURITY.md). Proven by `verify_hardening.sh` section D |
...
- [ ] Container egress backstop live (#311): `verify_hardening.sh` §D green — single first-matching `NC-EGRESS` jump ending in DROP, db-plane service denied off-host, allowlisted host reachable
