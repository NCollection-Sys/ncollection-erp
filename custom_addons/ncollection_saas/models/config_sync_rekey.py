...
class TenantConfigSyncRekey(models.Model):
    ...
    def _rekey_one(self, master):
        """Re-key ONE tenant. Returns (tenant, ok: bool, detail: str).
        ...
        if self.database_status != 'ready' or not db:
            return (self, False, 'skipped: no ready database')
        try:
    ...
    def _rekey_verify(self, db):
        ...
        if self.config_sync_state == 'ok':
            return (self, True, 'ok')
        return (self, False, 'key written but push still failing: %s' % (
    ...
    def _rekey_summary(self, results):
        ...
        for tenant, success, detail in results:
            tenant.message_post(body=self.env._(
                "Config-sync key re-keyed: %(outcome)s (%(detail)s)",
                outcome='OK' if success else 'FAILED', detail=detail))
        for tenant, _success, detail in failed:
            _logger.error("Config-sync re-key FAILED for %s: %s",
                          tenant.database_name, detail)
        _logger.info("Config-sync re-key finished: %s ok, %s failed",
                     len(ok), len(failed))
        if failed:
            message = self.env._(
                "Re-keyed %(ok)s tenant(s); %(bad)s FAILED: %(names)s. "
                "Failed tenants still present the OLD key — check each one's "
                "chatter before considering the rotation complete.",
                ok=len(ok), bad=len(failed),
                names=', '.join(t.database_name or '?' for t, _s, _d in failed))
        else:
            message = self.env._(
                "Re-keyed %(ok)s tenant(s); every one authenticated on the "
                "verification push.", ok=len(ok))
        return {
